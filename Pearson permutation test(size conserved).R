...
GSE=GSE[,-index]
GSE_gene<-as.character(rownames(GSE))
net_gene<-read.table("recon_net_symbol_degree.txt",header = T,sep = "\t")
all_gene_id<-as.character(intersect(net_gene[,2],rownames(GSE)))
trans=function(x){
  num=as.numeric(as.character(x))
  return(num)
}
GSE=apply(GSE,2,trans)
rownames(GSE)=GSE_gene
